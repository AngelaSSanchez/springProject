...
public interface UnRelatedDao extends JpaRepository<UnRelatedEntity, Integer>, UnRelatedExtended {
    ...
    List<UnRelatedEntity> findByNickLike(String nick);
    
    List<UnRelatedEntity> findByGenderEquals(Gender gender);
    ...
    @Query("select u.bornDate from other_name_for_unrelatedentity u where u.gender like ?1")
    List<GregorianCalendar> findBornDateByGender(Gender Gender);
    ...
    @Query(value = "SELECT * FROM other_name_for_unrelatedentity WHERE bornDate > ?1", nativeQuery = true)
    List<UnRelatedEntity> findByBornDate(GregorianCalendar day);
